...
const senderPool = [
    mailHelper.buildSender(mailConstants.MAIL_PROVIDER, mailConstants.MAIL_SUPPORT_USERNAME, mailConstants.MAIL_SUPPORT_PASSWORD),
];

const MAIL_SUPPORT_INDEX = 0;
const FORGET_PASSWORD_SUBJECT = "[FORGET PASSWORD] Your password just has been reset";
...
function sendTokenToUserMail(userEmail, token) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const emailHtml = [
          ...
          `<a href="http${ssl}://${host}:${port}${urlConstants.AUTH_FORGET_PASSWORD_API_URL}?token=${token}"><span class="str">link</span></a>`,
