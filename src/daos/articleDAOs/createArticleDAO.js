...
function createArticle(article, options = null) {
	return new Promise(function (resolve, reject) {
		...
		const query = [
			`INSERT INTO article (`,
			insertId,
			'name, journal, journalUrl, conference, `rank`, rankUrl, year, page_from, page_to, volume, issue, city, abstract,',
			`institution, department, type, month, day,`,
			`url_date_access,`,
			`ArXivID, DOI, ISBN, ISSN,`,
			`PMID, Scopus, PII, SGR,`,
			`project_id, citation_key, general_note, citationCount, googleScholarCitationCount,`,
			`created_at, updated_at, is_deleted`,
			`)`,
			'VALUES (?)',
		].join(' ');
